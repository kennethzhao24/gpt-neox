...
from megatron.model.custom_transformer import (
...
class OPTModelPipe(PipelineModule, torch.nn.Module):
    """
        OPT Model adapted for pipeline parallelism.
    ...
    def init_specs(self):
        ...
        if weight_tying:
            self.specs.append(
                TiedLayerSpec(
                    ...
                    EmbeddingPipe,
                    self.neox_args,
                    self.neox_args.word_embed_proj_dim,
            ...
            self.specs.append(
                LayerSpec(
                    EmbeddingPipe,
                    self.neox_args,
                    self.neox_args.word_embed_proj_dims,
                    self.neox_args.padded_vocab_size,
                    self.neox_args.max_position_embeddings,
                    self.neox_args.hidden_dropout,
                    self.init_method,
        ...
        if self.neox_args.h_list[0] != self.neox_args.word_embed_proj_dim:
            self.specs.append(
                LayerSpec(
                    ParallelLinearPipe,
                    args=self.args,
                    hidden_size=self.neox_args.word_embed_proj_dim,
                    output_size=self.neox_args.h_list[0],
        ...
        # Transformer layers by block
        for idx in range(self.neox_args.num_blocks):
            num_layers = self.neox_args.layers_per_block[idx]
            self.neox_args.hidden_size = self.neox_args.h_list[idx]
            self.neox_args.ffn_dim = self.neox_args.ffn_list[idx]
        ...
        if self.neox_args.h_list[-1] != self.neox_args.word_embed_proj_dim:
            self.specs.append(
                LayerSpec(
                    ParallelLinearPipe,
                    args=self.neox_args,
                    hidden_size=self.neox_args.hidden_size[-1],
                    output_size=self.neox_args.word_embed_proj_dim,
                    init_method=self.init_method,
                    parallel_output=self.parallel_output,
                    is_last_layer=False
                    )
        ...
        if weight_tying:
            ...
            self.specs.append(
                LayerSpec(
                    ParallelLinearPipe,
                    neox_args=self.neox_args,
                    input_size=self.neox_args.word_embed_proj_dims,
                    output_size=self.neox_args.padded_vocab_size,
                    init_method=self.init_method,
                    parallel_output=self.parallel_output,
                    is_last_layer=True,
                )
